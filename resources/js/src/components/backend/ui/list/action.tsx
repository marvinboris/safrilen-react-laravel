...
export default function Action({ item, resource, props }: ActionProps) {
    ...
    if (props) {
        ...
        if (role === 'user') {
            ...
            additionalContent = <>
                {(feature && feature.access.includes('u')) && <Link to={`/${role}/${resource}/${item.id}/edit`} className="mx-1">
                    <PencilIcon className="text-sky w-5" />
                </Link>}
                {(feature && feature.access.includes('d')) && <span className="mx-1"><Delete deleteAction={() => props.delete(item.id!)}><TrashIcon className="w-5 text-red cursor-pointer" /></Delete></span>}
            </>;
        }
