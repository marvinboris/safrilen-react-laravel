        ...
        <div className={classNames('relative overflow-hidden text-sm transition-all duration-200', navItemOpen ? 'max-h-96 py-2' : 'max-h-0')}>
            ...
            <div className='space-y-2.5'>
                {items.map(({ href: _href, label }) => <div key={`nav-item-${href}-items-${_href}`} className="relative ml-[64px]">
                    ...
                    <Link onClick={hideSideDrawer} className={classNames(location.pathname.startsWith(href + _href) ? "font-medium text-primary" : "", "transition-all duration-200")} to={href + _href}>{label}</Link>
                </div>)}
            </div>
        </div>
    </div> : <Link to={href} onClick={hideSideDrawer} className={classNames("flex items-center rounded-[12.5106px] w-full truncate transition-all duration-200", main ? 'text-white font-display bg-primary relative py-4 scale-[0.87] font-medium shadow-lg after:absolute after:inset-y-3 after:left-2 after:w-1 after:bg-orange after:rounded-xl after:shadow-md' : active ? 'bg-primary/10 text-primary font-medium py-3 mr-6 px-[11px]' : 'text-secondary-700 hover:text-primary px-[11px]')}>
        <div className={classNames(main ? 'mr-[11.68px] pl-[29.19px]' : 'mr-[22px]')}><Icon className={classNames(main ? 'text-white/20 w-4' : 'text-primary/50 w-6')} /></div>
