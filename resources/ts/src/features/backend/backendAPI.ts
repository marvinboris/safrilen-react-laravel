...
const backendAPI = {
    dashboard: async (role: string) => {
        const res = await axios.get(`/api/${role}/dashboard`)
        return res.data
    },
    ...
    show: (info: { role: string, resource: string }) => (id: number) => manageResource(info.role, info.resource, 'show', id),
    post: (info: { role: string, resource: string }) => (data: any) => manageResource(info.role, info.resource, 'post', data),
    patch: (info: { role: string, resource: string }) => (id: number | string, data: any) => manageResource(info.role, info.resource, 'patch', id, data),
    delete: (info: { role: string, resource: string }) => (id: number) => manageResource(info.role, info.resource, 'delete', id),
}
