...
export const manageResource = async (
    ...
    try {
        let url: string, page, show, search, res, id: number, pageElt, showElt, searchElt, data
        ...
        switch (type) {
            case 'index':
                ...
                show = params[1] || 10;
                search = params[2] || '';
                url = `/api/${role}/${root}?page=${page}&show=${show}&search=${search}`;
                res = await axios.get(url);
                return res.data

            case 'info':
                url = `/api/${role}/${root}/info`;
                res = await axios.get(url);
                return res.data

            case 'show':
                id = params[0];
                url = `/api/${role}/${root}/${id}`;
                res = await axios.get(url);
                return res.data
