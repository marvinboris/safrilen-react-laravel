...
export default function ManageAddOrEditPublications({ edit }: Props) {
    ...
    const { cms: { backend: { components: { form: { active, inactive } }, pages: { publications: { form } } } }, languages } = content!
            ...
            <div className="md:col-span-2">
                <div className="flex-1 grid gap-y-2 gap-x-4 md:grid-cols-2 overflow-auto">
                    {languages.map(l => <Fragment key={`language-${l.abbr}`}>
                        <Input icon={ChatBubbleLeftEllipsisIcon} id={`title-${l.abbr}`} className={l.abbr === state.translate ? "" : "hidden"} onChange={inputChangeHandler} value={(state.title as { [key: string]: string })[l.abbr]} name={`title[${l.abbr}]`} required validation={{ required: true }} label={form.title} />
                        <Input icon={ChatBubbleLeftEllipsisIcon} id={`description-${l.abbr}`} className={classNames('md:col-span-2', l.abbr === state.translate ? "" : "hidden")} onChange={inputChangeHandler} value={(state.description as { [key: string]: string })[l.abbr]} name={`description[${l.abbr}]`} required validation={{ required: true }} label={form.description} />
